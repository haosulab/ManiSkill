...
OBJECT_SEMANTIC_ID_MAPPING = None
SEMANTIC_ID_OBJECT_MAPPING = None
MOVEABLE_OBJECT_IDS = None

def load_ai2thor_metadata():
    global MOVEABLE_OBJECT_IDS, SEMANTIC_ID_OBJECT_MAPPING, OBJECT_SEMANTIC_ID_MAPPING

    if SEMANTIC_ID_OBJECT_MAPPING is not None:
        with open(
            str(
                Path(ASSET_DIR)
                / "scene_datasets/ai2thor/ai2thor-hab/configs/object_semantic_id_mappinag.json"
            ),
            "r",
        ) as f:
    ...
    if MOVEABLE_OBJECT_IDS is not None:
        MOVEABLE_OBJECT_IDS = [
            "Apple",
            "AppleSliced",
            "Tomato",

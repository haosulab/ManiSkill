...
- `--use-env-states`: For each time step $t$, after replaying the action at this time step and obtaining a new observation at $t+1$, ignore this observation, and instead set the environment state at time $t+1$ as the recorded environment state at time $t+1$ and then obtain the corresponding observation. This is necessary for successfully replaying trajectories for the tasks migrated from ManiSkill1.

...
python ppo_fast.py --env_id="PickCube-v1" \
  ...
  --total_timesteps=5_000_000 --eval_freq=100 --cudagraphs --exp-name="PickCube-v1-ppo"

python ppo_fast.py --env_id="PickCube-v1" --evaluate \
  --checkpoint=runs/PickCube-v1-ppo/final_ckpt.pt \
  --num_eval_envs=1024 --num-eval-steps=50 --eval_partial_reset
...
python ppo_fast.py --env_id="PushT-v1" \
  --num_envs=4096 --num-steps=16 --update_epochs=8 --num_minibatches=32 \
  --total_timesteps=25_000_000 --num-eval-steps=100 --gamma=0.99 --cudagraphs
...
class Agent_Seperate_MLPs(nn.Module):
    def __init__(self, envs, init_type):
        if init_type == "orthogonal":
            ...
            raise ValueError(f"Unknown init type {init_type}. Has to be either 'orthogonal' or 'uniform'")
...
class Agent_Shared_MLPs(nn.Module):
    def __init__(self, envs, init_type):
        ...
        self.actor_mean = layer_init(nn.Linear(256, np.prod(envs.single_action_space.shape)), std=0.01*np.sqrt(2))
        self.actor_logstd = nn.Parameter(torch.ones(1, np.prod(envs.single_action_space.shape)) * -0.5)
...
def run(
    ...
    if capture_video:
        ...
        if save_train_video_freq is not None:
            ...
            envs = RecordEpisode(envs, output_dir=f"runs/{run_name}/train_videos", save_trajectory=False, save_video_trigger=save_video_trigger, max_steps_per_video=num_steps, video_fps=30)
        eval_envs = RecordEpisode(eval_envs, output_dir=eval_output_dir, save_trajectory=evaluate, trajectory_name="trajectory", max_steps_per_video=num_eval_steps, video_fps=30)
    envs = ManiSkillVectorEnv(envs, num_envs, ignore_terminations=not partial_reset, **env_kwargs)
    eval_envs = ManiSkillVectorEnv(eval_envs, num_eval_envs, ignore_terminations=not partial_reset, **env_kwargs)
    ...
    print(f"####")
    print(f"num_iterations={num_iterations} num_envs={num_envs} num_eval_envs={num_eval_envs}")
    ...
    if action_transformation == "tanh":
        ...
        raise ValueError(f"Unknown action transformation {action_transformation}. Has to be either 'tanh' or 'clip'")
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        if iteration % eval_freq == 1:
            ...
            print(f"Evaluated {num_eval_steps * num_envs} steps resulting in {len(eps_lens)} episodes")
            if len(successes) > 0:
            ...
            if writer is not None:
                writer.add_scalar("charts/eval_episodic_return_per_step", returns.mean(), global_step)
                writer.add_scalar("charts/eval_episodic_return_per_episode", returns.mean(), iteration)
        ...
        for step in range(0, num_steps):
            ...
            next_obs, reward, terminations, truncations, infos = envs.step(action_transformation(action))
            ...
            if "final_info" in infos:
                ...
                writer.add_scalar("charts/episodic_return_per_step", episodic_return, global_step)
                writer.add_scalar("charts/episodic_return_per_episode", episodic_return, iteration)
        ...
        writer.add_scalar("charts/rollout_fps", num_envs * num_steps / rollout_time, global_step)

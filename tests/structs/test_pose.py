...
def test_pose_create_with_p():
    ...
    pose = Pose.create_from_pq(p=[
        [1, 0, 2],
        [1, 0, -2]
    ])
    assert torch.isclose(pose.raw_pose, torch.tensor([
        [1, 0, 2, 1.0, 0, 0, 0],
        [1, 0, -2, 1.0, 0, 0, 0]
    ]
    )).all()
...
def test_pose_create_with_q():
    ...
    pose = Pose.create_from_pq(p=[1,2,3], q=[
        [w, x, y, z],
        [w2, x2, y2, z2]
    ])
    assert torch.isclose(pose.raw_pose, torch.tensor([[1, 2, 3, w, x, y, z], [1, 2, 3, w2, x2, y2, z2]])).all()

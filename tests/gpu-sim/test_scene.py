...
def test_parallel_actors():
    ...
    assert np.isclose(raw_pose[0], np.array([2, 0, 1, 1, 0, 0, 0])).all(), "pose in scene 0 is wrong"
    assert np.isclose(raw_pose, raw_pose[0]).all(), "poses are not synced across sub scenes"
    raw_pose = to_numpy(scene.actors["cube-1"].pose.raw_pose)
    assert np.isclose(raw_pose[0], np.array([5, 5, 5, 1, 0, 0, 0])).all(), "pose in scene 0 is wrong"
    assert np.isclose(raw_pose, raw_pose[0]).all(), "poses are not synced across sub scenes"
    
    scene.px.step()
    scene._gpu_fetch_all()
    assert np.isclose(to_numpy(scene.actors["cube-1"].get_raw_data()["linear_vel"]), np.array([0, 0, -0.0981])).all(), "linear velocity in scene 0 after one sim step is wrong"

def test_env_step():
    num_envs = 4
    env: BaseEnv = gym.make("PickCube-v0", num_envs=num_envs, render_mode="human", control_mode="pd_joint_pos", sim_freq=200)
    env.reset(seed=0)
    env=env.unwrapped
    scene=env._scene

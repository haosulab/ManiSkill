...
ManiSkill supports MacOS and can let you run CPU simulation and the standard rendering. GPU simulation is currently not yet supported on MacOS. To get started we strongly recommend setting up a python [uv](https://docs.astral.sh/uv/) or  [conda/mamba](https://mamba.readthedocs.io/en/latest/index.html) environment with python version >= 3.10. You can then install ManiSkill as follows

```
# using uv
uv venv -p python3.11
uv pip install mani_skill-nightly
```
...
conda activate ms3
pip install mani_skill-nightly
```

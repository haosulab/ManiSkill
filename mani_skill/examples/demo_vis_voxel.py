...
def parse_args(args=None):
    ...
    parser.add_argument("--segmentation", type=bool, default=False, action=argparse.BooleanOptionalAction, help="Whether or not to include voxel segmentation estimations")
...
def render_filtered_voxels(voxel_grid, zf=1.0, rotation_amount=60, env_id="PushCube-v1"):
    if (env_id in ["TwoRobotPickCube-v1", "TwoRobotStackCube-v1"]):
        # different envs have different floor id
        flood_id = 32
    else:
        flood_id = 17
...
def render_all_voxels(voxel_grid, zf=1.0, rotation_amount=60):
    vis_voxel_grid = voxel_grid.permute(0, 4, 1, 2, 3).detach().cpu().numpy()
    rendered_img = visualise_voxel(vis_voxel_grid[0],
                                None,
                                None,
...
def main(args):
    ...
    if not args.use_default:
        ...
        # Not overriding obs_mode_config. This works only for some tabletop tasks like PushCube-v1 and PickCube-v1
        env: BaseEnv = gym.make(
            args.env_id,
            obs_mode="voxel",
        )
    ...
    obs, _ = env.reset()
    print(f"Shape of the voxel grids: {obs['voxel_grid'].shape}")
    while True:
        action = env.action_space.sample()
        obs, reward, terminated, truncated, info = env.step(action)
        voxel_grid = obs["voxel_grid"]
        if args.segmentation:
            # if voxel seg id is available, filter the floors and then render the scene voxels
            img = render_filtered_voxels(voxel_grid, zf, args.rotation_amount, args.env_id)
        else:
            # if voxel seg id not available, just render the scene voxels
            img = render_all_voxels(voxel_grid, zf, args.rotation_amount)
        plt.axis('off')
        plt.imshow(img)
        plt.show()

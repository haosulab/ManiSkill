...
def parse_args(args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="rgb+depth",
        help="Can be rgb or rgb+depth, rgb+normal, albedo+depth etc. Which ever image-like textures you want to visualize can be tacked on",
    )
    parser.add_argument(
        "--shader",
        default="default",
        type=str,
        help="Change shader used for all cameras in the environment for rendering. Default is 'minimal' which is very fast. Can also be 'rt' for ray tracing and generating photo-realistic renders. Can also be 'rt-fast' for a faster but lower quality ray-traced renderer",
    )
    parser.add_argument(
        "--num-envs",
        type=int,
        default=1,
        help="Number of environments to run. Used for some basic testing and not visualized",
    )
    parser.add_argument(
        "--cam-width",
        type=int,
        help="Override the width of every camera in the environment",
    )
    parser.add_argument(
        "--cam-height",
        type=int,
        help="Override the height of every camera in the environment",
    )
    parser.add_argument(
        "-s",
        "--seed",
        ...
        for cam in obs["sensor_data"].keys():
            for texture in obs["sensor_data"][cam].keys():
                if obs["sensor_data"][cam][texture].dtype == torch.uint8:
                    ...
                    data = common.to_numpy(obs["sensor_data"][cam][texture][0]).astype(
                        np.float32
                    )

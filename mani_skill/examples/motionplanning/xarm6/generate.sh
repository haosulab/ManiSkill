...
do
    python -m mani_skill.examples.motionplanning.xarm6.run --env-id $env_id \
      --traj-name="trajectory_simplest" --only-count-success --save-video -n 1 \
      --shader="rt" # generate sample videos
    mv demos/$env_id/motionplanning/0.mp4 demos/$env_id/motionplanning/sample.mp4
    python -m mani_skill.examples.motionplanning.xarm6.run --env-id $env_id --traj-name="trajectory_simplest" -n 1000 --only-count-success
done
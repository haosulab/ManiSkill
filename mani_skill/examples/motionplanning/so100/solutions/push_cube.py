...
def solve(env: PushCubeEnv, seed=None, debug=False, vis=False):
    ...
    reach_pose = sapien.Pose(p=env.obj.pose.sp.p + np.array([-0.025, 0, env.cube_half_size]), q=env.agent.tcp_pose.sp.q)
    planner.move_to_pose_with_screw(reach_pose)

    # -------------------------------------------------------------------------- #
    # Move to goal pose
    # -------------------------------------------------------------------------- #
    goal_pose = sapien.Pose(p=env.goal_region.pose.sp.p + np.array([0, 0, env.cube_half_size]),q=env.agent.tcp_pose.sp.q)

...
def solve(env: ScoopParticlesEnv, seed=None, debug=False, vis=False):
    ...
    offset = sapien.Pose([0, -0.1, 0])
    grasp_pose = grasp_pose * (offset)

    # -------------------------------------------------------------------------- #
    # Reach
    # -------------------------------------------------------------------------- #
    reach_pose = grasp_pose * sapien.Pose([0, 0, 0])  # 15 cm above grasp pose
    ...
    approach_offset = sapien.Pose([0, 0.05, 0.0])
    ...
    lift_pose.set_q(approach_pose.q)  # Maintain grasp orientation
    res = planner.move_to_pose_with_screw(lift_pose)
    if res == -1: return res

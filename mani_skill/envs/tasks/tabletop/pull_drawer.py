...
@register_env("PullDrawer-v1", max_episode_steps=100)
class PullDrawerEnv(BaseEnv):
    ...
    def _load_agent(self, options: dict):
        return super()._load_agent(
            options, initial_agent_poses=sapien.Pose(p=[-0.615, 0, 0])
        )
    ...
    def _load_scene(self, options: dict):
        ...
        for y_sign in [-1, 1]:
            ...
            drawer.add_box_collision(
                sapien.Pose(
                    [-self.inner_width / 2 - self.handle_offset / 2, support_y, 0]
                ),
                half_size=[
                    self.handle_offset / 2,
                    self.handle_thickness / 2,
                    self.handle_thickness / 2,
                ],
            )
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        handle_offset = torch.tensor(
            [-self.inner_width / 2 - self.handle_offset, 0, 0],
            device=drawer_link_pose.device,
        )
        ...
        desired_q = torch.tensor([0.5, 0.5, 0.5, 0.5], device=device).expand(
            batch_size, 4
        )
        ...
        success_mask = info.get(
            "success", torch.zeros_like(pulling_reward, dtype=torch.bool)
        )

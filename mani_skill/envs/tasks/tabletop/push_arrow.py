...
# @register_env("PushArrow-v1", max_episode_steps=100)
# class PushArrowEnv(BaseEnv):
#     """
#     **Task Description:**
...
#     arrow_spawnbox_ylength = 0.3

#     # translation of the spawnbox from goal tee as upper left of spawnbox
#     arrow_spawnbox_xoffset = -0.1
#     arrow_spawnbox_yoffset = -0.1
...
#         )  # same as mani_skill.utils.building.actors.common - goal targetcom_vert

#         def create_arrow(name="arrow", target=False, base_color=TARGET_RED):
#             # This will be a bit different, that is you will get the arrow from online, then use those dimensions calculate the CM
#             # 

#             # dimensions of boxes that make tee
#             # box2 is same as box1, except (3/4) the length, and rotated 90 degrees
...
#             # box1_half_h = 0.142 / 2 #(l/2)
#             # box1_half_w = 0.031 / 2 #(l/8)
#             # half_thickness = 0.01982 / 2 if not target else 1e-4

#             # we have to center tee at its com so rotations are applied to com
#             # vertical block is (3/4) size of horizontal block's base, so
#             # Assuming that the angle of the horizontal block at the base is 60 degrees.
#             # center of mass is (1*com_horiz + (3/4)*com_vert) / (1+(3/4))
#             # # center of mass is (1*(0,0)) + (3/4)*(0,(.025+.15)/2)) / (1+(3/4)) = (0,0.0375)
#             # com_y = (0.1133-0.071) # Above the CM of the first block
...
#             # )
#             if not target:
#                 builder.add_visual_from_file("/home/prajwal/Downloads/ManiSkill-main/mani_skill/assets/push_arrow/arrow.glb")
#                 builder.add_multiple_convex_collisions_from_file("/home/prajwal/Downloads/ManiSkill-main/mani_skill/assets/push_arrow/arrow.glb", decomposition="coacd")
#                 builder.set_initial_pose(sapien.Pose(p=[0, 0, 0.1]))
#                 return builder.build(name=name)
#             else:
#                 builder.add_visual_from_file("mani_skill/assets/push_arrow/arrow_target.glb")
#                 builder.set_initial_pose(sapien.Pose(p=[0, 0, 0.1]))
#                 return builder.build_kinematic(name=name)

#         self.arrow = create_arrow(name="Arrow", target=False)
#         # self.goal_arrow = create_arrow(
#         #     name="goal_Arrow",
#         #     target=True,
...
#         #     0,
#         #     0.1133-0.071,
#         # )  # With respect to the CM of the lower Block
#         # # box1 = torch.tensor(
...
#         # # )
#         # # Convert the CM to the origin.
#         # # box1[:, 1] -= self.center_of_mass[1]
#         # # box2[:, 1] -= self.center_of_mass[1]

#         # # 2D Plane vertices of  all the shapes, wrt origin.which is also the CM of the entire object
#         # vertices = torch.tensor(
#         #     [[0, 0.13], # The topmost vertex
#         #      [-0.030, 0.031], # Arrow top part left vertex
#         #      [0.030, 0.031], # Arrow top part right vertex
#         #      [-0.016, 0.031], # Arrow bottom part top left vertex
#         #      [0.016, 0.031], # Arrow bottom part top right vertex
#         #      [-0.016, -0.113], # Arrow bottom part left vertex
#         #      [0.016, -0.113] # Arrow bottom part right vertex
#         #     ]
#         # )
...
#         # # convert the arrows to indices
#         # vertices *= (res / 2) / uv_half_width
#         # vertices += res / 2
#         # vertices = vertices.long()
#         # self.arrow_render = torch.zeros(res, res)
#         # # Drawing the arrow by connecting the vertices
#         # # image map has flipped x and y, set values in transpose to undo
#         # # self.arrow_render.T[box1[0, 0] : box1[1, 0], box1[2, 1] : box1[0, 1]] = 1
#         # # self.arrow_render.T[box2[0, 0] : box2[1, 0], box2[2, 1] : box2[0, 1]] = 1
#         # # image map y is flipped of xy plane, flip to unflip
#         # self.arrow_render[vertices[3, 0]  : vertices[4, 0], vertices[5, 1] : vertices[3, 1]] = 1

#         # def world_to_grid(point):
#         #     return ((point * (res / 2) / uv_half_width) + (res / 2)).long()

#         # # Draw arrow head
#         # head_start = (vertices[1]+vertices[2])/2
#         # head_l = (vertices[0,1]-vertices[1,1])
#         # head_w = vertices[2,0]-vertices[1,0]
#         # start = int(head_start[1]-head_l)
#         # end = int(head_start[1])
#         # for i in range(start, end):
#         #     span = int(((i - (head_start[1]-head_l)) / head_l) * (head_w/2))
#         #     self.arrow_render[i, int(head_start[0])-span:int(head_start[0])+span] = 1

#         # self.arrow_render = self.arrow_render.flip(0).to(self.device)
...
#         #     self.device
#         # )  # this is just a 3x3 matrix (2d homogeneous transform)
...
#         alphas = self.quat_to_z_euler(quats) # Essentially, given the quaternion, we extract the rotation around the z-axis
#         # constructing rot matrix for rotation around z with the alpha values
...
#     #     arrow_to_world_trans = self.quat_to_zrot(
#     #         self.arrow.pose.q
#     #     )  # should be (b,3,3) rot matrices
#     #     # This is the translation component
#     #     arrow_to_world_trans[:, 0:2, 2] = self.arrow.pose.p[
#     #         :, :2
#     #     ]  # should be (b,3,3) rigid trans matrices

#     #     # these matrices convert egocentric 3d tee to 2d goal tee frame
#     #     arrow_to_goal_trans = (
#     #         self.world_to_goal_trans @ arrow_to_world_trans
#     #     )  # should be (b,3,3) rigid trans matrices

#     #     # making homogeneous coords of uv map to apply transformations to view tee in goal tee frame
#     #     b = arrow_to_world_trans.shape[0]
#     #     res = self.uv_grid.shape[1] # This is what I will have to generate. UV_grid
#     #     homo_uv = self.homo_uv

#     #     # finally, get uv coordinates of tee in goal tee frame
#     #     arrows_in_goal_frame = (arrow_to_goal_trans @ homo_uv.view(3, -1)).view(
#     #         b, 3, res, res
#     #     )
#     #     # convert from homogenious coords to normal coords
#     #     arrows_in_goal_frame = arrows_in_goal_frame[:, 0:2, :, :] / arrows_in_goal_frame[
#     #         :, -1, :, :
#     #     ].unsqueeze(
#     #         1
#     #     )  #  now (b,2,res,res)
...
#     #     arrow_coords = arrows_in_goal_frame[:, :, self.arrow_render == 1].view(
#     #         b, 2, -1
...
#     #     arrow_indices = (
#     #         (arrow_coords * ((res / 2) / self.uv_half_width) + (res / 2))
#     #         .long()
#     #         .view(b, 2, -1)
#     #     )  #  (b,2,num_points_in_tee)
...
#     #     # for batch indexing
#     #     num_arrow_pixels = arrow_indices.shape[-1]
#     #     batch_indices = (
#     #         torch.arange(b).view(-1, 1).repeat(1, num_arrow_pixels).to(self.device)
#     #     )
...
#     #     invalid_xs = (arrow_indices[:, 0, :] < 0) | (arrow_indices[:, 0, :] >= self.res)
#     #     invalid_ys = (arrow_indices[:, 1, :] < 0) | (arrow_indices[:, 1, :] >= self.res)
#     #     arrow_indices[:, 0, :][invalid_xs] = 0
#     #     arrow_indices[:, 1, :][invalid_xs] = 0
#     #     arrow_indices[:, 0, :][invalid_ys] = 0
#     #     arrow_indices[:, 1, :][invalid_ys] = 0

#     #     final_renders[batch_indices, arrow_indices[:, 0, :], arrow_indices[:, 1, :]] = 1
...
#     #     intersection = (
#     #         (final_renders.bool() & self.arrow_render.bool()).sum(dim=[-1, -2]).float()
#     #     )
#     #     goal_area = self.arrow_render.bool().sum().float()
...
#             # self.goal_arrow.set_pose(
#             #     Pose.create_from_pq(
#             #         p=target_region_xyz,
#             #         q=euler2quat(np.pi/2, 0, self.goal_z_rot),
#             #     )
#             # )
...
#             # q[:, 1]
#             # q[:, -1] = (q_euler_angle / 2).sin()# Rotation about x-axis as well.
#             q_euler_angle = torch.rand(b) * (2 * torch.pi)
...
@register_env("PushArrow-v1", max_episode_steps=50)
class PushArrowEnv(BaseEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    def _load_scene(self, options: dict):
        ...
        self.arrow = self.load_glb_as_actor(self.scene, 
                                            # "/home/prajwal/Downloads/ManiSkill-main/mani_skill/assets/push_arrow/arrow.glb",

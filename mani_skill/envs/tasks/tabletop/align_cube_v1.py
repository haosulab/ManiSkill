...
@register_env("AlignCube-v1", max_episode_steps=100)
class AlignCubeEnv_v1(BaseEnv):
    ...
    def evaluate(self):
        ...
        xy_flag_CA = torch.abs(offset_AC[..., 1]) <= 0.005
        z_flag_CA = torch.abs(offset_AC[..., 2]) <= 0.005
        is_cubeC_next_to_cubeA = torch.logical_and(xy_flag_CA, z_flag_CA)
        ...
        task_1_done = is_cubeA_next_to_cubeB * is_cubeA_static * (~is_cubeA_grasped)
        task_2_done = is_cubeC_next_to_cubeA * is_cubeC_static * (~is_cubeC_grasped)
        success = task_1_done * task_2_done
        ...
        return {
            ...
            "is_cubeC_next_to_cubeA": is_cubeC_next_to_cubeA,
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        def _compute_reward(cube_1, cube_2, is_cube1_grasped, is_cube1_static,is_cube1_next_to_cube2,task_done):
            ...
            goal_xyz = torch.hstack(
                [(cube_2_pos[:, 0] + self.cube_half_size[0] * 2)[:, None], cube_2_pos[:, 1:]]
            )
            ...
            reward[info[is_cube1_next_to_cube2]] = (
                6 + (ungrasp_reward + static_reward) / 2.0
            )[info[is_cube1_next_to_cube2]]
            reward[info[task_done]] =8.0
            return reward
        task_1_reward=_compute_reward(self.cubeA,self.cubeB,"is_cubeA_grasped","is_cubeA_static","is_cubeA_next_to_cubeB","task_1_done")
        task_2_reward=_compute_reward(self.cubeC,self.cubeA,"is_cubeC_grasped","is_cubeC_static","is_cubeC_next_to_cubeA","task_2_done")
        return task_1_reward+task_2_reward*info["task_1_done"]
        


    def compute_normalized_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        return self.compute_dense_reward(obs=obs, action=action, info=info) / 16.0

...
class Kinematics:
    ...
    def compute_ik(
        ...
        delta_solver_config: dict = dict(type="levenberg_marquardt", alpha=1.0),
    ):
        ...
        Args:
            ...
            delta_solver_config (dict): configuration for the delta IK solver. Default is `dict(type="levenberg_marquardt", alpha=1.0)`. type can be one of     "levenberg_marquardt" or "pseudo_inverse". alpha is a scaling term applied to the delta joint positions generated by the solver.

        """
        # TODO (stao): this function can really be simplified a lot. It's either cpu IK via pinnochio, or LM / pseudo inverse for batched IK.
        # for batched IK given target pose, we compute delta pose and solve accordingly. For batched ik given a delta pose, we skip computing the delta pose and solve
        # directly.
        if self.use_gpu_ik:
            ...
            if not use_delta_ik_solver:
                ...
                return q0[:, self.qmask] + delta_solver_config[
                    "alpha"

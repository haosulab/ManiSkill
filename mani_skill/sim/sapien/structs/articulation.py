...
class SapienArticulation(SapienBaseStruct[physx.PhysxArticulation], Articulation):
    ...
    """
    Map from a set of joints of this articulation and the indexing torch tensor to use for setting
    drive targets in GPU sims.
    """
    ...
    def __str__(self):
        return (
            f"<{self.name}: struct of type {self.__class__}; managing "
            f"{self._num_objs} {self._objs[0].__class__} objects>"
        )
    ...
    def create_from_physx_articulations(
        ...
        to manage different articulations use the .merge function.
        """
        shared_name = "_".join(physx_articulations[0].name.split("_")[1:])

        # NOTE (stao): This is a little bit of a non-standard way to use @classmethod style creation
        # functions for dataclasses however generating the Joint and Link objects relies on a
        # reference to the articulation properties so this is done as a convenience
        self = cls(
        ...
        for articulation in physx_articulations:
            if _process_links:
                assert num_links == len(articulation.links) and num_joints == len(
                    ...
                    "Gave different physx articulations. Articulation object created via "
                    "create_from_physx_articulations can only manage the same articulations, "
                    "not different ones. Use merge instead if you want to manage different "
                    "articulations"
                )
        ...
        for links in all_links_objs:
            ...
            if wrapped_link.is_root.any():
                ...
                if not _process_links:
                    # if not processing links we break early so that we only processed the root link
                    # which is always shared between articulations
                    break
    ...
    def _data_index(self):
        """
        Returns a tensor of the indices of the articulation in the GPU simulation for the physx_cuda
        backend.
        """
        return torch.tensor(
    ...
    def get_first_collision_mesh(
        ...
        """
        Returns the collision mesh of the first managed articulation object. Note results of this
        are not cached or optimized at the moment so this function can be slow if called too often.
        Some articulations have no collision meshes, in which case this function returns None.
    ...
    def get_collision_meshes(
        ...
        """
        Returns the collision mesh of each managed articulation object. Note results of this are
        not cached or optimized at the moment so this function can be slow if called too often.

        Args:
            to_world_frame (bool): Whether to transform the collision mesh pose to the world frame
            first_only (bool): Whether to return the collision mesh of just the first articulation
                managed by this object. If True, this also returns a single Trimesh.Mesh object
                instead of a list. This can be useful for efficiency reasons if you know ahead
                of time all of the managed actors have the same collision mesh
        """

        assert not self.merged, (
            "Currently you cannot fetch collision meshes of merged articulations as merged "
            "articulations only share a root link"
        )
        if self.sim.gpu_sim_enabled:
            assert self.sim._gpu_sim_initialized, (
                "During GPU simulation link pose data is not accessible until after "
                "initialization, and link poses are needed to get the correct collision mesh of an "
                "entire articulation"
            )
        else:
    ...
    def get_first_visual_mesh(self, to_world_frame: bool = True) -> trimesh.Trimesh:
        """
        Returns the visual mesh of the first managed articulation object. Note results of this are
        not cached or optimized at the moment so this function can be slow if called too often
        """
    ...
    def get_visual_meshes(
        ...
        """
        Returns the visual mesh of each managed articulation object. Note results of this are not
        cached or optimized at the moment so this function can be slow if called too often
        """
        assert not self.merged, (
            "Currently you cannot fetch visual meshes of merged articulations as merged "
            "articulations only share a root link"
        )
        if self.scene.gpu_sim_enabled:
            assert self.scene._gpu_sim_initialized, (
                "During GPU simulation link pose data is not accessible until after "
                "initialization, and link poses are needed to get the correct visual mesh of an "
                "entire articulation"
            )
        else:
            self._objs[0].pose = self._objs[0].pose
    ...
    def get_net_contact_impulses(self, link_names: Union[list[str], Tuple[str]]):
        """
        Get net contact impulses for several links together. This should be faster compared to
        using link.get_net_contact_impulses on each link.

        Returns impulse vector of shape (N, len(link_names), 3) where N is the number of
        environments.
        """
        if self.sim.gpu_sim_enabled:
    ...
    def get_net_contact_forces(self, link_names: Union[list[str], Tuple[str]]):
        """Get net contact forces for several links together. This should be faster compared to
        using link.get_net_contact_forces on each link.
        """
    ...
    def get_joint_target_indices(
        ...
        of joint indexes or a 1D list of ArticulationJoint objects.

        Internally the given input is made to a tuple for a cache key and is used to cache results
        for fast lookup in the future, particularly for large-scale GPU simulations.
        """
            ...
            if isinstance(joint_indices[0], SapienArticulationJoint):
                for joint in joint_indices:
                    assert joint.articulation == self, (
                        ...
                        "joints from this articulation"
                    )

                vals = [
                    x.active_index[0] for x in joint_indices
                ]  # active_index on joint is batched but it should be the same value across all
                # managed joint objects
    ...
    def drive_targets(self):
        """
        The current drive targets of the active joints. Also known as the target joint positions.
        Returns a tensor of shape (N, M) where N is the number of environments and M is the number
        of active joints.
        """
    ...
    def drive_velocities(self):
        """
        The current drive velocity targets of the active joints. Also known as the target joint
        velocities. Returns a tensor of shape (N, M) where N is the number of environments and M is
    ...
    # def create_fixed_tendon(self, link_chain: list[PhysxArticulationLinkComponent],
    # coefficients: list[float], recip_coefficients: list[float], rest_length: float = 0,
    # offset: float = 0, stiffness: float = 0, damping: float = 0,
    ...
    def find_joint_by_name(self, arg0: str) -> SapienArticulationJoint:
        if self.merged:
            raise RuntimeError(
                ...
                "articulations of different dofs"
            )
    ...
    def get_link_incoming_joint_forces(self):
        """
        Returns the incoming joint forces, referred to as spatial forces, for each link, with shape
        (N, M, 6), where N is the number of environments and M is the number of links.

        Spatial forces are complex to describe and we recommend you see the physx documentation for
        how they are computed:
        https://nvidia-omniverse.github.io/PhysX/physx/5.6.0/docs/Articulations.html#link-incoming-joint-force

        The mth spatial force refers to the mth link, and to find a particular link you can find the
        Link object first then use it's index attribute.
    ...
    def set_joint_drive_targets(
        ...
        """
        Set drive targets on active joints given joints. For GPU simulation only joint_indices
        argument is supported, which should be a 1D list of the active joint indices in the
        articulation.

        joints argument will always be used when possible and is the recommended approach. On CPU
        simulation the joints argument is required, joint_indices is not supported.
        """
        if self.sim.gpu_sim_enabled:
    ...
    def set_joint_drive_velocity_targets(
        ...
        """
        Set drive velocity targets on active joints given joints. For GPU simulation only
        joint_indices argument is supported, which should be a 1D list of the active joint indices
        in the articulation.

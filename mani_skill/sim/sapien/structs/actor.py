...
class SapienActor(PhysxRigidDynamicComponentStruct[sapien.Entity]):
    ...
    At the moment, on GPU and CPU one can query pose, linear velocity, and angular velocity
    easily
    ...
    It is necessary to track this pose to ensure the actor is still at the correct pose once
    ...
    def __str__(self):
        return (
            f"<{self.name}: struct of type {self.__class__}; managing {self._num_objs} "
    ...
    def create_from_entities(
        ...
        bodies = cast(
            list[physx.PhysxRigidDynamicComponent],
            [
        ...
        # Objects with collision shapes have either PhysxRigidDynamicComponent (Kinematic, Dynamic)
        # or PhysxRigidStaticComponent (Static)
        px_body_type = "static"
    ...
    def set_state(self, state: Array, env_idx: Optional[torch.Tensor] = None):
        if self.physics_sim.gpu_sim_enabled:
            if env_idx is not None:
                prev_reset_mask = self.physics_sim._reset_mask.clone()
                # safe guard against setting the wrong states
                self.physics_sim._reset_mask[:] = False
                self.physics_sim._reset_mask[env_idx] = True
            ...
            if env_idx is not None:
                self.physics_sim._reset_mask = prev_reset_mask
        else:
    ...
    def has_collision_shapes(self):
        assert not self.merged, (
            "Check if a merged actor has collision shape is not supported as the managed "
            "objects could all be very different"
        )
        return (
            len(
                self._objs[0]
    ...
    def hide_visual(self):
        ...
        For GPU simulation, currently this is implemented by moving the actor very far away as
        visiblity cannot be changed on the fly. As a result we do not permit hiding and showing
        visuals of objects with collision shapes as this affects the actual simulation. Note that
        this operation can also be fairly slow as we need to run px.gpu_apply_rigid_dynamic_data
        and px.gpu_fetch_rigid_dynamic_data.
        """
        ...
        if self.physics_sim.gpu_sim_enabled:
            ...
            temp_pose[..., :3] += 99999
            self.pose = temp_pose
            self.px.gpu_apply_rigid_dynamic_data()  # type: ignore
            self.px.gpu_fetch_rigid_dynamic_data()  # type: ignore
        else:
            for obj in self._objs:
                obj.find_component_by_type(
                    sapien.render.RenderBodyComponent
                ).visibility = 0  # type: ignore
    ...
    def get_first_collision_mesh(
        ...
        """
        Returns the collision mesh of the first managed actor object. Note results of this are
        not cached or optimized at the moment so this function can be slow if called too often.
        Some actors have no collision meshes, in which case this function returns None
    ...
    def get_collision_meshes(
        ...
        """
        Returns the collision mesh of each managed actor object. Note results of this are not
        cached or optimized at the moment so this function can be slow if called too often.
        Some actors have no collision meshes, in which case this function returns an empty list.

        Args:
            to_world_frame (bool): Whether to transform the collision mesh pose to the
                world frame
            first_only (bool): Whether to return the collision mesh of just the first actor
    ...
    def per_scene_id(self):
        """
        Returns a int32 torch tensor of the actor level segmentation ID for each managed actor
        object.
        """
        return torch.tensor(
    ...
    def apply_force(self, force: Array):
        """
        Apply an instantaneous external force to this actor in Newtons to the body's center
        of mass. Once called no need to call any gpu_apply_x functions as this handles it for you.
        """

        if self.physics_sim.gpu_sim_enabled:
            self.px.cuda_rigid_body_force.torch()[  # type: ignore
                self._body_data_index, :3
            ] = common.to_tensor(force, device=self.device)
            self.px.gpu_apply_rigid_dynamic_force()  # type: ignore
        else:
            for body in self._bodies:
                body.add_force_at_point(
                    force=force,
                    point=(body.pose * body.cmass_local_pose).p,  # type: ignore
                )
    ...
    def remove_from_scene(self):
        if self.physics_sim.gpu_sim_enabled:
            raise RuntimeError(
                "Cannot physically remove object from scene during GPU simulation. "
                "This can only be done in CPU simulation. If you wish to remove an object "
                "physically, the best way is to move the object far away."
            )
        else:
        ...
        if self.physics_sim.gpu_sim_enabled:
            if self.px_body_type == "static":
                ...
                if self.hidden:
                    ...
                    if self.physics_sim.parallel_in_single_scene:
                        new_xyzs = (
                            raw_pose[:, :3]
                            - self.physics_sim.scene_offsets[self._scene_idxs]
                        )
    ...
    def pose(self, arg1: Union[Pose, sapien.Pose, Array]) -> None:
        if self.physics_sim.gpu_sim_enabled:
            ...
            if self.hidden:
                self.before_hide_pose[
                    self.physics_sim._reset_mask[self._scene_idxs]
                ] = arg1
                return
            if self.physics_sim.parallel_in_single_scene:
                if len(arg1.shape) == 1:
                    arg1 = arg1.view(1, -1)
                mask = self.physics_sim._reset_mask[self._scene_idxs]
                new_xyzs = (
                    arg1[:, :3] + self.physics_sim.scene_offsets[self._scene_idxs[mask]]
                )
            ...
            if isinstance(arg1, sapien.Pose):
                ...
                if isinstance(arg1, Pose) and len(arg1.shape) == 2:
                    ...
                    arg1 = to_sapien_pose(arg1)  # type: ignore
                    for obj in self._objs:
                        obj.pose = arg1

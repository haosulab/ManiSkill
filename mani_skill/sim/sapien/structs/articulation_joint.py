...
class SapienArticulationJoint(
    ...
    At the moment, all of the same joints across all sub scenes are restricted
    ...
    def drive_target(self) -> torch.Tensor:
        ...
        if self.sim.gpu_sim_enabled:
            return self.sim.px.cuda_articulation_target_qpos.torch()[  # type: ignore
                self._data_index,
                self.active_index,
            ]
        else:
    ...
    def limits(self) -> torch.Tensor:
        # TODO (stao): create a decorator that caches results once
        # gpu sim is initialized for performance

...
# NOTE (stao): commented out functions are functions that are not confirmed to be working in
# the wrapped class but the original class has
...
class RenderCamera:
    ...
    def create(
        ...
        return cls(
            _render_cameras=render_cameras, sim=sim, name=shared_name, mount=mount
        )
        ...
        if self._cached_local_pose is None:
            if self.sim.gpu_sim_enabled:
                ...
                self._cached_local_pose = Pose.create(
                    common.to_tensor(ps), device=self.sim.render_device_torch
                )
            else:
    ...
    def get_model_matrix(self):
        ...
        if self.sim.gpu_sim_enabled:
            ...
            b = len(pose.raw_pose)
            qmat = torch.zeros((b, 4, 4), device=self.sim.render_device_torch)
            qmat[:, :3, :3] = quaternion_to_matrix(pose.q)
            qmat[:, -1, -1] = 1
            pmat = torch.eye(4, device=self.sim.render_device_torch)[None, ...].repeat(
                len(qmat), 1, 1
            )
            pmat[:, :3, 3] = pose.p
            res = pmat @ qmat
    ...
    def get_picture(self, names: Union[str, list[str]]) -> list[torch.Tensor]:
        if isinstance(names, str):
            names = [names]
        if self.sim.gpu_sim_enabled and not self.sim.scene.parallel_in_single_scene:
            ...
            if self.sim._render_device.is_cuda():  # type: ignore
                return [

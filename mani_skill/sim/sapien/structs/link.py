...
class SapienLink(
    ...
    """
    map from user-defined mesh groups (e.g. "handle" meshes for cabinets) to a list of
    trimesh.Trimesh objects corresponding to each physx link object managed here
    """
    ...
    def create(
        ...
        return cls(
            ...
            _bodies=physx_links,  # type: ignore
        )
    ...
    def merge(cls, links: list["SapienLink"], name: str):
        ...
        # remove articulation reference as it does not make sense and is only used to
        # instantiate some properties like the physx system
    ...
    def get_visual_meshes(
        ...
        """
        Returns the visual mesh of each managed link object. Note results of this are not
    ...
    def generate_mesh(
        ...
        """
        Generates mesh objects (trimesh.Trimesh) for each managed physx link given a filter
        and saves them to self.meshes[mesh_name] in addition to returning them here.
        """
        # TODO (stao): should we create a Mesh wrapper? that is basically trimesh.Trimesh but
        # all batched.
    ...
    # TODO (stao): In future can we have some shared nice functions like getting center of
    # mass of desired meshes (e.g. handle mesh?)
    ...
    def bbox(
        ...
        # First we need to pre-compute the bounding box of the link at 0. This will be slow
        # the first time
        bboxes = []
        for link, link_render_shapes in zip(self._objs, self.render_shapes):
            meshes = []
    ...
    def set_collision_group_bit(self, group: int, bit_idx: int, bit: Union[int, bool]):
        ...
        Args:
            group (int): the collision group to set the bit for. Typically you only need to
            use group 2 to disable collision checks between links to enable faster
            simulation.
    ...
    def per_scene_id(self) -> torch.Tensor:
        """
        Returns a int32 torch tensor of the link level segmentation ID for each managed link
        object.
        """
        return torch.tensor(
    ...
    def index(self) -> torch.Tensor:
        """
        The indexes of the managed link objects in their respective articulations. NOTE that
        these do not correspond with position in the qpos and qvel of articulations. For that
        index use index_q
        """
        return torch.tensor(

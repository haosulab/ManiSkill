...
class ManiSkillDataset(Dataset):
    def __init__(
        self, dataset_file: str,device: torch.device, load_count=-1
    ) -> None:
        ...
        for eps_id in tqdm(range(load_count)):
            ...
            self.depth.append(trajectory["obs"]["sensor_data"]["base_camera"]["depth"][:-1])
            self.actions.append(trajectory["actions"])

            # print(trajectory.keys())
        self.rgb = np.vstack(self.rgb)
        self.depth = np.vstack(self.depth)
    ...
    def __getitem__(self, idx):
        ...
        out["rgb"] = torch.from_numpy(self.rgb[idx]).float().to(device=self.device)
        out["state"] = torch.from_numpy(self.states[idx]).float().to(device=self.device)
        out["depth"] = torch.from_numpy(self.depth[idx]).float().to(device=self.device)
        return out
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim):
        super().__init__()
        self.encoder = PlainConv(in_channels=4, out_dim=256, max_pooling=False, inactivated_output=False)
    ...
    def forward(self, rgb, depth, state):
        rgb = rgb / 255.0 # (B, H, W, 3*k)
        depth = depth / 1024 # (B, H, W, 1*k)
    ...
    @torch.no_grad()
    def act(self, obs):
        return self(obs["rgb"], obs["depth"], obs["state"]).cpu().data.numpy()
...
def make_eval_envs(env, num_envs, control_mode,gpu= False):
    if gpu:
        env = gym.make(
            env,
            num_envs=num_envs,
...
def process_obs_gpu(obs):
    out = {}
    out["rgb"] = obs["sensor_data"]["base_camera"]["rgb"]
    out["depth"] = obs["sensor_data"]["base_camera"]["depth"]
    out["state"] = torch.hstack([
        flatten_state_dict_with_space(obs["agent"]),
        flatten_state_dict_with_space(obs["extra"])
    ])
    return out

def process_obs(obs, device):
    out = {}
    out["rgb"] = torch.from_numpy(obs["sensor_data"]["base_camera"]["rgb"]).to(device=device)
    out["depth"] = torch.from_numpy(obs["sensor_data"]["base_camera"]["depth"]).to(device=device)
    out["state"] = torch.from_numpy(np.hstack([
        flatten_state_dict_with_space(obs["agent"]),
        flatten_state_dict_with_space(obs["extra"])
    ])).to(device=device)
    return out
...
if __name__ == "__main__":
    ...
    actor = Actor(ds.states.shape[1], envs.single_action_space.shape[0]).to(device=device)
    ...
    for i, batch in enumerate(data_loader):
        ...
        if i % args.eval_freq == 0:
            ...
            for _ in range(100):
                if gpu:
                    inpt = process_obs_gpu(obs)
                else:
                    inpt = process_obs(obs, device)

                obs, reward, terminated, truncated, info = envs.step(actor.act(inpt))
                if "success" in info:
            ...
            if len(successes) > 0:
                ...
                log_dict["charts/eval_success_at_end"] = s.mean()
                success_once += s

...
for seed in ${seeds[@]}
do
    python ppo_cpu.py --env_id="PickCube-v1" \
    --num_envs=16 --num_eval_envs=4 --num-steps=200 --num-eval-steps=200 --update_epochs=8 --num_minibatches=8 \
    --total_timesteps=1_500_000 --seed=${seed} --wandb_entity="stonet2000" --track \
    --exp-name="ppo-PickCube-v1-rgb-${seed}-walltime_efficient-cpu_sim"
done
"""
...
if __name__ == "__main__":
    ...
    max_episode_steps = 50#gym_utils.find_max_episode_steps_value(envs._env)
    logger = None
            ...
            if "final_info" in infos:
                ...
                with torch.no_grad():
                    for i in range(args.num_envs):
                        if done_mask[i]:
                            final_values[step, i] = agent.get_value(infos["final_observation"][i]).view(-1)

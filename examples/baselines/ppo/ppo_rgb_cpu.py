"""
python ppo_rgb_cpu.py --env_id="PushCube-v1" \
  --num_envs=16 --num-eval-envs=4 --update_epochs=8 --num_minibatches=8 \
  --total_timesteps=1_000_000 --eval_freq=15 --num-steps=200 --num-eval-steps=50
"""
...
if __name__ == "__main__":
    ...
    def cpu_make_env(env_id, ignore_terminations=False, env_kwargs = dict(), record_video=False):
        def thunk():
            env = gym.make(env_id, reconfiguration_freq=1, **env_kwargs)
            if record_video:
                env = RecordEpisode(env, output_dir=f"runs/{run_name}/eval_videos", save_trajectory=False, video_fps=30)
            ...
            return env
        return thunk
    vector_cls = lambda x : gym.vector.AsyncVectorEnv(x, context="forkserver")

    # env setup
    # env_kwargs = dict(obs_mode="state", control_mode="pd_joint_delta_pos", render_mode="rgb_array", sim_backend="cpu")
    ...
    # env setup
    env_kwargs = dict(obs_mode="rgb", control_mode="pd_joint_delta_pos", render_mode=args.render_mode, sim_backend="cpu")
    envs = vector_cls([cpu_make_env(args.env_id, env_kwargs=env_kwargs) for _ in range(args.num_envs)])
    eval_envs = vector_cls([cpu_make_env(args.env_id, ignore_terminations=True, env_kwargs=env_kwargs, record_video= True if i == 0 else False) for i in range(args.num_eval_envs)])
    ...
    max_episode_steps = 50#gym_utils.find_max_episode_steps_value(envs)
    logger = None
    ...
    print(f"####")
    next_obs = common.to_tensor(next_obs, device=device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if iteration % args.eval_freq == 1:
            ...
            for _ in range(args.num_eval_steps):
                with torch.no_grad():
                    ...
                    if "final_info" in eval_infos:
                        ...
                        for finfo in eval_infos["final_info"]:
                            if finfo is not None:
                                for k, v in finfo["episode"].items():
            ...
            for k, v in eval_metrics.items():
                ...
                # mean = torch.stack(v).floeat().mean()
        ...
        for step in range(0, args.num_steps):
            ...
            next_done = torch.from_numpy(next_done).to(device).to(torch.float32)
            # next_done = torch.logical_or(terminations, truncations).to(torch.float32)
            rewards[step] = torch.from_numpy(reward).to(device).view(-1) * args.reward_scale

            if "final_info" in infos:
                final_info = infos["final_info"]
                done_mask = infos["_final_info"]
                for finfo in final_info:
                    if finfo is not None:
                        for k, v in finfo["episode"].items():
                ...
                with torch.no_grad():
                    for i in range(args.num_envs):
                        if done_mask[i]:
                            final_values[step, i] = agent.get_value(common.batch(common.to_tensor(infos["final_observation"][i], device=device))).view(-1)
        ...
        with torch.no_grad():
            next_value = agent.get_value(common.to_tensor(next_obs, device=device)).reshape(1, -1)

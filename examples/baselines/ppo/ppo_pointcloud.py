...
from mani_skill.utils.wrappers.flatten import FlattenActionSpaceWrapper, FlattenPointCloudObservationWrapper
            ...
            for k, v in element_space.items():
                if isinstance(v, gym.spaces.dict.Dict):
                    ...
                    dtype = (torch.float32 if v.dtype in (np.float32, np.float64) else
                            ...
                            torch.bool if v.dtype == bool else
                            v.dtype)
...
class PointNetBackbone(nn.Module):
    def __init__(self, sample_obs, feat_dim = 128):
        super().__init__()
        self.mlp = nn.Sequential(
            nn.Linear(6, 64),  nn.ReLU(), nn.BatchNorm1d(64),
            nn.Linear(64,128), nn.ReLU(), nn.BatchNorm1d(128),
            nn.Linear(128,feat_dim), nn.ReLU(), nn.BatchNorm1d(feat_dim),
        )
        ...
        if "state" in sample_obs:
            ...
            self.state_fc = nn.Linear(state_size, feat_dim)
            self.out_features += feat_dim
    ...
    def forward(self, obs):
        ...
        pc_feat = self.mlp(pc)
        pc_feat = global_max_pool(pc_feat, batch)

        if "state" in obs:
            state = obs["state"]
            state_feat = self.state_fc(state)
            feat = torch.concat([pc_feat, state_feat], axis=-1)
        else:
            feat = pc_feat
        return feat
...
class Agent(nn.Module):
    def __init__(self, envs, sample_obs):
        super().__init__()
        self.feature_net = PointNetBackbone(sample_obs=sample_obs)
        print("num params: ", [v.numel() for v in self.feature_net.parameters()])
...
if __name__ == "__main__":
    ...
    envs = FlattenPointCloudObservationWrapper(envs, rgb=True, state=args.include_state)
    eval_envs = FlattenPointCloudObservationWrapper(eval_envs, rgb=True, state=args.include_state)

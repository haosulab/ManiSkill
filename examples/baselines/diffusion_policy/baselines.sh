...
do
  ...
  demo_path=~/.maniskill/demos/PushT-v1/${demo_type}/trajectory.state.pd_ee_delta_pos.physx_cuda.h5
  if [ -f "$demo_path" ]; then
    python train.py --env-id PushT-v1 \
      --demo-path $demo_path \
      --control-mode "pd_ee_delta_pos" --sim-backend "physx_cuda" --num-demos ${demos} --max_episode_steps 100 --num_eval_envs 100 \
      --total_iters 30000 --act_horizon 2 \

...
for demo_type in "motionplanning" "rl"
do
  demo_path=~/.maniskill/demos/PickCube-v1/${demo_type}/trajectory.rgb.pd_ee_delta_pos.physx_cpu.h5
  if [ -f "$demo_path" ]; then
    python train_rgbd.py --env-id PickCube-v1 \
      --demo-path $demo_path \
      --control-mode "pd_ee_delta_pos" --sim-backend "physx_cpu" --num-demos ${demos} --max_episode_steps 100 \
      --total_iters 30000 --no-depth \
  ...
  demo_path=~/.maniskill/demos/PushCube-v1/${demo_type}/trajectory.rgb.pd_ee_delta_pos.physx_cpu.h5
  if [ -f "$demo_path" ]; then
  ...
  demo_path=~/.maniskill/demos/StackCube-v1/${demo_type}/trajectory.rgb.pd_ee_delta_pos.physx_cpu.h5
  if [ -f "$demo_path" ]; then
  ...
  demo_path=~/.maniskill/demos/DrawTriangle-v1/${demo_type}/trajectory.rgb.pd_ee_delta_pose.physx_cpu.h5
  if [ -f "$demo_path" ]; then
    python train.py --env-id DrawTriangle-v1 \
      --demo-path $demo_path \
      --control-mode "pd_ee_delta_pos" --sim-backend "physx_cpu" --num-demos ${demos} --max_episode_steps 300 \
      --total_iters 100000 --no-depth \
      --exp-name diffusion_policy-DrawTriangle-v1-state-${demos}_motionplanning_demos-${seed} \
      --demo_type=${demo_type} --track # additional tag for logging purposes on wandb
  else
    echo "Demo path $demo_path does not exist. Skipping DrawTriangle-v1 for ${demo_type}."
  fi
done
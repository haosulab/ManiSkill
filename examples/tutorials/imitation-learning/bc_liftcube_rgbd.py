...
def parse_args():
    ...
    parser.add_argument("-d", "--demos", type=str, help="path to demonstration dataset .h5py file")
    parser.add_argument(
        "--logs",
        type=str,
    ...
    parser.add_argument("--steps", type=int, help="numbr of training steps", default=10000)
...
def main(args):
    ...
    env = RecordEpisode(env, output_dir=osp.join(log_path, f"{env_id}/videos"))
    

    dataset = ManiSkill2Dataset(f"demos/rigid_body/{env_id}/trajectory.rgbd.pd_ee_delta_pose.h5")
    dataloader = DataLoader(dataset, batch_size=100, num_workers=4, pin_memory=True, drop_last=True, shuffle=True)
    ...
    def evaluate_policy(env, policy):
        ...
        return success_rate
    # policy.load_state_dict(state_dict=th.load("ckpt_18000.pt")["policy"])
    # sr = evaluate_policy(env, policy)
    # print(sr)
    # exit()
    from torch.utils.tensorboard import SummaryWriter
    writer = SummaryWriter(osp.join(log_path, f"{env_id}"))
    ...
    while steps < iterations:
        ...
        if epoch_loss < best_epoch_loss:
            ...
            save_model(policy, "ckpt_best.pt")
        if epoch % 50 == 0:
            print("Evaluating")
        ...
        epoch += 1
    save_model(policy, "ckpt_latest.pt")

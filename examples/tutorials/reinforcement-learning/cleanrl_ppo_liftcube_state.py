...
def make_env(env_id, idx, capture_video, run_name, gamma):
    def thunk():
        if capture_video and idx == 0:
            env = gym.make(env_id, render_mode="rgb_array", control_mode="pd_joint_delta_pos")
            env = gym.wrappers.RecordVideo(env, f"videos/{run_name}")
        else:
            env = gym.make(env_id, control_mode="pd_joint_delta_pos")
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        #         lifted = torch.zeros(args.num_eval_envs, device=device)
        ...
        #             next_obs, reward, terminations, truncations, infos = eval_envs.step((agent.get_action(next_obs, deterministic=True)).cpu().numpy())
        #             # rgbs = eval_envs.render()
        #             # images.append(rgbs)
        #             eval_returns += reward
        #             eval_eps_lens += 1
        ...
        # returns = torch.zeros(args.num_envs, dtype=torch.float, device=device)

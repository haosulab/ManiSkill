...
inline CUDA_CALLABLE bool isnan(float x) { return ::isnan(x); }
inline CUDA_CALLABLE void adj_isnan(float, float &, bool &) {}
...
template<typename T>
inline CUDA_CALLABLE T atomic_min(T* buf, T value)
{
#if defined(WP_CPU)
    T old = buf[0];
    buf[0] = min(old, value);
    ...
    return atomicMin(buf, value);
#endif
}

template<typename T>
inline CUDA_CALLABLE T atomic_max(T* buf, T value)
{
#if defined(WP_CPU)
    T old = buf[0];
    buf[0] = max(old, value);
